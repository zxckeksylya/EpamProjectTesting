    ...
    <div class="card-body">
        <table class="table table-light table-stripped" id="answers">
            ...
            <tr>
                ...
                <th scope="col">список ответов для теста</th>
                <th scope="col">Новый ответ</th>
            </tr>
            </thead>
            <tbody>

            <c:forEach items="${quests}" var="item" varStatus="currentNumber">
                <tr>
                    <th scope="row"><c:out value="${currentNumber.count}"/></th>
                    <td><c:out value="${item.textOfQuest}"/></td>

                    <td>
                        <form class="form-inline my-0 my-lg-0" action="<c:url value="/system_of_testing"/>" method="POST">
                            <input type="hidden" name="command" value="to_answers_list_by_id"/>
                    ...
                    <td>
                        <form class="form-inline my-0 my-lg-0" action="<c:url value="/system_of_testing"/>" method="POST">
                            <input type="hidden" name="command" value="to_add_answer"/>
                            <input type="hidden" name="quest_id" value="${item.id}">
                            <button type="submit" class="btn btn-dark">Новый ответ</button>
                        </form>
                    </td>
                </tr>
            </c:forEach>
