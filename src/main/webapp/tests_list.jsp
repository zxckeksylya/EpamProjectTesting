...
<div class="card bg-dark text-white form-client-list">
    ...
    <div class="card-body">
        <table class="table table-light table-stripped" id="tests">
            <thead class="thead-dark">
            <tr>
                <th scope="col">#</th>
                <th scope="col">Название теста</th>
                <th scope="col">список вопросов для теста</th>
                <th scope="col">Новый вопрос</th>
                <th scope="col">пройти тест</th>
            </tr>
            </thead>
            <tbody>

            <c:forEach items="${tests}" var="item" varStatus="currentNumber">
                <tr>
                    <th scope="row"><c:out value="${currentNumber.count}"/></th>
                    <td><c:out value="${item.nameOfTest}"/></td>
                    <td>
                        <form class="form-inline my-0 my-lg-0" action="<c:url value="/system_of_testing"/>" method="POST">
                            <input type="hidden" name="command" value="to_quests_list_by_id"/>
                            <input type="hidden" name="test_id" value="${item.id}">
                            <button type="submit" class="btn btn-dark">Ответы</button>
                        </form>
                    </td>
                    <td>
                        <form class="form-inline my-0 my-lg-0" action="<c:url value="/system_of_testing"/>" method="POST">
                            <input type="hidden" name="command" value="to_add_quest"/>
                            <input type="hidden" name="test_id" value="${item.id}">
                            <button type="submit" class="btn btn-dark">Новый вопрос</button>
                        </form>
                    </td>
                    <td>
                        <form class="form-inline my-0 my-lg-0" action="<c:url value="/system_of_testing"/>" method="POST">
                            <input type="hidden" name="command" value="to_add_testing"/>
                            <input type="hidden" name="test_id" value="${item.id}">
                            <button type="submit" class="btn btn-dark">Пройти тест</button>
                        </form>
                    </td>
                </tr>
            </c:forEach>
